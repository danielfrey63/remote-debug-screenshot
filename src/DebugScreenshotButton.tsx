...
export type DebugScreenshotButtonProps = {
  ...
  size?: number; // px
  tooltip?: string;
  style?: React.CSSProperties; // allow consumer overrides
  onUploadStart?: () => void;
...
function DebugIcon({ playing }: { playing: boolean }) {
  // A simple camera-bug style icon
  return (
    <svg viewBox="0 0 24 24" width="100%" height="100%" aria-hidden="true">
      <circle cx="12" cy="12" r="7" fill={playing ? '#0ea5e9' : '#6366f1'} opacity="0.15"/>
      <rect x="6" y="8" width="12" height="8" rx="2" fill="none" stroke="#e5e7eb" strokeWidth="1.5"/>
      <circle cx="12" cy="12" r="2.5" fill="none" stroke="#e5e7eb" strokeWidth="1.5"/>
      <path d="M18 9l2-1.5v9L18 15" fill="none" stroke="#e5e7eb" strokeWidth="1.5"/>
    </svg>
  );
}

async function loadHtml2Canvas(): Promise<(el: HTMLElement, opts?: Record<string, unknown>) => Promise<HTMLCanvasElement>> {
...
export const DebugScreenshotButton: React.FC<DebugScreenshotButtonProps> = ({
  uploadUrl = '/upload-screenshot',
  clientId,
  source = 'qr-debug-screenshot',
  componentName = 'DebugScreenshotButton',
  ...
  const upload = useCallback(async (dataURL: string, note: string) => {
    setBusy(true);
    onUploadStart?.();
    try {
  ...
  const handleClick = useCallback(async () => {
    ...
    if (canDisplayCapture) {
      ...
      try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        stream = await (navigator.mediaDevices as any).getDisplayMedia({ video: { displaySurface: 'browser' }, audio: false });
    ...
    try {
      ...
      if (canvas.width > targetWidth || canvas.height > targetHeight) {
        ...
        safe.width = targetWidth; safe.height = targetHeight;
        const sctx = safe.getContext('2d');
        if (!sctx) throw new Error('Canvas 2D context unavailable (downscale).');
